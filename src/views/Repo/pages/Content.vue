 ...
 * @LastEditors: iuukai
 * @LastEditTime: 2023-06-03 18:07:50
...
const beforeRoute = async (to, from, next) => {
	try {
		...
		repoStore.setContents(contents)
...
export default defineComponent({
	...
	setup() {
		...
		const treeList = isArray(repoStore.getContents)
			? repoStore.getContents.sort((a, b) => a.type.localeCompare(b.type))
			: []
		// readme
		...
		async function getRepoData() {
			try {
				...
				const responseList = await Promise.all([getFileContent(params), ...portalRequestList])
