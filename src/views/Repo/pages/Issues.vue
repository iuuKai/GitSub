 ...
 * @LastEditors: iuukai
 * @LastEditTime: 2023-07-09 00:14:08
...
<template>
	<div class="issues_body">
		...
		<a-card :style="{ marginBottom: '10px' }">
				...
				<template #renderItem="{ item }">
					<a-list-item>
						<a-comment :style="{ width: '100%' }">
							...
							<template #avatar>
								<a-avatar :src="item?.user?.avatar_url" :alt="item?.user?.login" />
							</template>
							<template #content>
								<MarkDownPreview
									:isDark="isDark"
			...
			<a-list v-else item-layout="horizontal" :data-source="issuses">
				<template #renderItem="{ item }">
					<a-list-item>
						<a-list-item-meta>
							<template #title>
...
const { isDark, issuses, details, comments, likes, dislikes, action } = toRefs(state)
const value = ref()
...
watch(
	() => route.params,
	async val => {
		const { owner, repo, number } = val
		if (number) {
			const list = await getRepoIssueComments({ owner, repo, number })
			state.details = list[0]
			state.comments = list.flat()
		} else {
			const list = await getRepoIssues({ owner, repo })
			state.issuses = list
		}
	},
	{ immediate: true }
